"""
CoinGlass Coin Taker Buy/Sell Volume History API Tool

This module provides functionality to fetch coin taker buy/sell volume history data
...
Usage Example:
    from tools.coinglass.coin_taker_buy_sell_volume_history import get_coin_taker_buy_sell_volume_history
    
    # Get coin taker buy/sell volume history for Bitcoin
    data = get_coin_taker_buy_sell_volume_history(symbol="BTC", interval="1h")
"""
...
def get_coin_taker_buy_sell_volume_history(symbol="BTC", interval="1h", exchange_list="Binance,OKX,Bybit", start_time=None, end_time=None):
    """
    Fetch coin taker buy/sell volume history from CoinGlass API.
    ...
    Returns:
        list or pandas.DataFrame: List of dictionaries or DataFrame containing coin taker buy/sell volume history data
    ...
    Example:
        data = get_coin_taker_buy_sell_volume_history("BTC", "1h", "Binance,OKX,Bybit")
        print(data[:5])  # Show first 5 coin taker buy/sell volume history records
    """
    # Validate API key
    ...
    # Prepare API request
    url = "https://open-api-v4.coinglass.com/api/futures/aggregated-taker-buy-sell-volume/history"
...
if __name__ == "__main__":
    ...
    # It fetches coin taker buy/sell volume history from CoinGlass API.
    # Example usage:
    #   python coin_taker_buy_sell_volume_history.py --symbol BTC
    #   python coin_taker_buy_sell_volume_history.py --symbol ETH --interval 4h --output_format csv
    import argparse
    import json
    
    parser = argparse.ArgumentParser(description="Fetch coin taker buy/sell volume history from CoinGlass API")
    ...
    try:
        ...
        data = get_coin_taker_buy_sell_volume_history(
            symbol=args.symbol,
