"""
CoinGlass Funding Rate Volume Weight OHLC History API Tool

This module provides functionality to fetch volume-weighted funding rate OHLC history data
...
Usage Example:
    from tools.coinglass.funding_rate_vol_weight_ohlc_history import get_funding_rate_vol_weight_ohlc_history
    
    # Get volume-weighted funding rate OHLC history for Bitcoin
    data = get_funding_rate_vol_weight_ohlc_history(symbol="BTC", interval="1h")
"""
...
def get_funding_rate_vol_weight_ohlc_history(symbol="BTC", interval="1h", start_time=None, end_time=None):
    """
    Fetch volume-weighted funding rate OHLC history from CoinGlass API.
    ...
    Returns:
        list or pandas.DataFrame: List of dictionaries or DataFrame containing volume-weighted funding rate OHLC data
    ...
    Example:
        data = get_funding_rate_vol_weight_ohlc_history("BTC", "1h")
        print(data[:5])  # Show first 5 volume-weighted funding rate OHLC records
    """
    # Validate API key
    ...
    # Prepare API request
    url = "https://open-api-v4.coinglass.com/api/futures/funding-rate/vol-weight-history"
...
if __name__ == "__main__":
    ...
    # It fetches volume-weighted funding rate OHLC history from CoinGlass API.
    # Example usage:
    #   python funding_rate_vol_weight_ohlc_history.py --symbol BTC --interval 1h
    #   python funding_rate_vol_weight_ohlc_history.py --symbol ETH --interval 4h --output_format csv
    import argparse
    import json
    
    parser = argparse.ArgumentParser(description="Fetch volume-weighted funding rate OHLC history from CoinGlass API")
    ...
    try:
        ...
        data = get_funding_rate_vol_weight_ohlc_history(
            symbol=args.symbol,
