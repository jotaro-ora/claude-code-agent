"""
CoinGlass Whale Hyperliquid Position API Tool

This module provides functionality to fetch Hyperliquid whale position data
...
Usage Example:
    from tools.coinglass.whale_hyperliquid_position import get_whale_hyperliquid_position
    
    # Get Hyperliquid whale positions
    data = get_whale_hyperliquid_position()
"""
...
def get_whale_hyperliquid_position(symbol="BTC", limit=100):
    """
    Fetch Hyperliquid whale positions from CoinGlass API.
    ...
    Returns:
        list or pandas.DataFrame: List of dictionaries or DataFrame containing whale position data
    ...
    Example:
        data = get_whale_hyperliquid_position("BTC", 50)
        print(data[:5])  # Show first 5 whale position records
    """
    # Validate API key
...
if __name__ == "__main__":
    ...
    # It fetches Hyperliquid whale positions from CoinGlass API.
    # Example usage:
    #   python whale_hyperliquid_position.py --symbol BTC --limit 50
    #   python whale_hyperliquid_position.py --symbol ETH --limit 100 --output_format csv
    import argparse
    import json
    
    parser = argparse.ArgumentParser(description="Fetch Hyperliquid whale positions from CoinGlass API")
    parser.add_argument('--symbol', type=str, default='BTC', help='Cryptocurrency symbol (default: BTC)')
    parser.add_argument('--limit', type=int, default=100, help='Number of records to return (default: 100)')
    ...
    try:
        ...
        data = get_whale_hyperliquid_position(symbol=args.symbol, limit=args.limit)
