"""
CoinGlass Liquidation Exchange List API Tool

This module provides functionality to fetch liquidation exchange list data
...
Usage Example:
    from tools.coinglass.liquidation_exchange_list import get_liquidation_exchange_list
    
    # Get liquidation exchange list
    data = get_liquidation_exchange_list()
"""
...
def get_liquidation_exchange_list(time_range="24h"):
    """
    Fetch liquidation exchange list from CoinGlass API.
    
    Args:
        time_range (str): Time range (e.g., "4h", "24h", "7d")
    
    Returns:
        list or pandas.DataFrame: List of dictionaries or DataFrame containing liquidation exchange list data
    ...
    Example:
        data = get_liquidation_exchange_list("24h")
        print(data[:5])  # Show first 5 liquidation exchange list records
    """
    # Validate API key
    ...
    # Prepare API request
    url = "https://open-api-v4.coinglass.com/api/futures/liquidation/exchange-list"
...
if __name__ == "__main__":
    ...
    # It fetches liquidation exchange list from CoinGlass API.
    # Example usage:
    #   python liquidation_exchange_list.py --time_range 24h
    #   python liquidation_exchange_list.py --time_range 7d --output_format csv
    import argparse
    import json
    
    parser = argparse.ArgumentParser(description="Fetch liquidation exchange list from CoinGlass API")
    parser.add_argument('--time_range', type=str, default='24h', help='Time range (default: 24h)')
    ...
    try:
        ...
        data = get_liquidation_exchange_list(time_range=args.time_range)
