"""
CoinGlass Open Interest Exchange List API Tool

This module provides functionality to fetch open interest by exchange list data
...
Usage Example:
    from tools.coinglass.open_interest_exchange_list import get_open_interest_exchange_list
    
    # Get open interest by exchange list for Bitcoin
    data = get_open_interest_exchange_list(symbol="BTC")
"""
...
def get_open_interest_exchange_list(symbol="BTC"):
    """
    Fetch open interest by exchange list from CoinGlass API.
    
    Args:
        symbol (str): Cryptocurrency symbol (e.g., "BTC", "ETH")
    
    Returns:
        list or pandas.DataFrame: List of dictionaries or DataFrame containing open interest by exchange data
    ...
    Example:
        data = get_open_interest_exchange_list("BTC")
        print(data[:5])  # Show first 5 exchange open interest records
    """
    # Validate API key
...
if __name__ == "__main__":
    ...
    # It fetches open interest by exchange list from CoinGlass API.
    # Example usage:
    #   python open_interest_exchange_list.py --symbol BTC
    #   python open_interest_exchange_list.py --symbol ETH --output_format csv
    import argparse
    import json
    
    parser = argparse.ArgumentParser(description="Fetch open interest by exchange list from CoinGlass API")
    ...
    try:
        ...
        data = get_open_interest_exchange_list(symbol=args.symbol)
