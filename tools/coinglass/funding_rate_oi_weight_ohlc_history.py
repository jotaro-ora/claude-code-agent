"""
CoinGlass Funding Rate OI Weight OHLC History API Tool

This module provides functionality to fetch OI-weighted funding rate OHLC history data
...
Usage Example:
    from tools.coinglass.funding_rate_oi_weight_ohlc_history import get_funding_rate_oi_weight_ohlc_history
    
    # Get OI-weighted funding rate OHLC history for Bitcoin
    data = get_funding_rate_oi_weight_ohlc_history(symbol="BTC", interval="1h")
"""
...
def get_funding_rate_oi_weight_ohlc_history(symbol="BTC", interval="1h", start_time=None, end_time=None):
    """
    Fetch OI-weighted funding rate OHLC history from CoinGlass API.
    ...
    Returns:
        list or pandas.DataFrame: List of dictionaries or DataFrame containing OI-weighted funding rate OHLC data
    ...
    Example:
        data = get_funding_rate_oi_weight_ohlc_history("BTC", "1h")
        print(data[:5])  # Show first 5 OI-weighted funding rate OHLC records
    """
    # Validate API key
...
if __name__ == "__main__":
    ...
    # It fetches OI-weighted funding rate OHLC history from CoinGlass API.
    # Example usage:
    #   python funding_rate_oi_weight_ohlc_history.py --symbol BTC --interval 1h
    #   python funding_rate_oi_weight_ohlc_history.py --symbol ETH --interval 4h --output_format csv
    import argparse
    import json
    
    parser = argparse.ArgumentParser(description="Fetch OI-weighted funding rate OHLC history from CoinGlass API")
    ...
    try:
        ...
        data = get_funding_rate_oi_weight_ohlc_history(
            symbol=args.symbol,
