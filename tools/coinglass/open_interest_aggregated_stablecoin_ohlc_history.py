"""
CoinGlass Open Interest Aggregated Stablecoin OHLC History API Tool

This module provides functionality to fetch aggregated stablecoin open interest OHLC history data
...
Usage Example:
    from tools.coinglass.open_interest_aggregated_stablecoin_ohlc_history import get_open_interest_aggregated_stablecoin_ohlc_history
    
    # Get aggregated stablecoin open interest OHLC history for Bitcoin
    data = get_open_interest_aggregated_stablecoin_ohlc_history(symbol="BTC", interval="1h")
"""
...
def get_open_interest_aggregated_stablecoin_ohlc_history(symbol="BTC", interval="1h", exchange_list="Binance,OKX,Bybit", start_time=None, end_time=None):
    """
    Fetch aggregated stablecoin open interest OHLC history from CoinGlass API.
    ...
    Returns:
        list or pandas.DataFrame: List of dictionaries or DataFrame containing aggregated stablecoin open interest OHLC data
    ...
    Example:
        data = get_open_interest_aggregated_stablecoin_ohlc_history("BTC", "1h", "Binance,OKX,Bybit")
        print(data[:5])  # Show first 5 aggregated stablecoin open interest OHLC records
    """
    # Validate API key
...
if __name__ == "__main__":
    ...
    # It fetches aggregated stablecoin open interest OHLC history from CoinGlass API.
    # Example usage:
    #   python open_interest_aggregated_stablecoin_ohlc_history.py --symbol BTC --interval 1h
    #   python open_interest_aggregated_stablecoin_ohlc_history.py --symbol ETH --interval 4h --exchange_list Binance,OKX --output_format csv
    import argparse
    import json
    
    parser = argparse.ArgumentParser(description="Fetch aggregated stablecoin open interest OHLC history from CoinGlass API")
    ...
    try:
        ...
        data = get_open_interest_aggregated_stablecoin_ohlc_history(
            symbol=args.symbol,
